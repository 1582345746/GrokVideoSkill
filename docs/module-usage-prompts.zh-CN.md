# 模块使用话术（v2.2.0 native-director）

本文是可以直接复制给 Codex 的模块级标准话术。所有示例都使用占位符，不包含任何真实 Key。默认原则是“先规划和预检，用户批准后才创建可能收费的上游任务”。

所有模块默认使用一张连续的全画面（`single-full-frame`）。分屏、三联画和漫画格只对明确要求的特殊镜头启用，并且必须在该镜头设置 `allow_multi_panel=true`；不要把“三段画面”写成所有视频的通用风格。竖屏多人 T2V 会先经过布局风险门禁，严格人物一致性优先使用单画面关键帧 I2V。

先区分渲染媒介和主体来源。实际来源真人使用 `real-person`，AI 原创的真人镜头风格人物使用 `synthetic-human`，写实动漫、三渲二或相似真人的 CG 角色使用 `human-like-fictional`。自动判断有疑义时先展示素材并确认，再生成。
...
导演核心不是连续剧专属模块。它会把目标先拆成剧情节拍，再为每个镜头编译 `shot_role`、景别、机位、单一连续动作、表演变化、环境运动、声音意图、剪辑出口和有效入出点。单镜头生成可比最终使用时多一点素材，编辑器默认从 `edit_in` 到 `edit_out` 取有效片段，主动避开片尾叹气、冻结和无动机收尾。

建议按任务选择导演模式：

| 任务 | 导演模式 | 重点覆盖 |
| --- | --- | --- |
| 普通单镜头 | `single-shot` | 一个主题动作，快速 I2V/T2V |
| 微电影、家庭、爱情、科幻 | `cinematic-short` | 钩子、建立、升级、反应、转折和结尾钩子 |
| 两人交流 | `dialogue-scene` | 建立、过肩、说话者特写、听者反应、打断和沉默 |
| 无对白电影感 | `silent-cinema` | 空镜、道具插入、环境声和视觉因果 |
| 打斗、追逐、灾难 | `action-scene` | 发现、出招、闪避、撞击、反应、结果 |
| 喜剧 | `comedy-scene` | 误会、节拍、克制表演和反应，不靠反复摆表情 |
| 广告/表演/新闻 | `product-ad`、`performance`、`news-report` | 产品卖点、动作完成度、证据对应和示意标识 |
...
| `cinematic-short` | 微电影、家庭、爱情、科幻等多镜头短片 | 剧情节拍、景别变化、有效剪辑出口 |
| `dialogue-scene` | 两人或多人交流 | 建立、过肩、说话者特写、听者反应和打断 |
| `silent-cinema` | 无对白电影感场景 | 空镜、环境声、道具插入和视觉叙事 |
| `action-scene` | 打斗、追逐、灾难升级 | 单动作节点、撞击、反应、空间轴线 |
| `comedy-scene` | 情境喜剧与误会 | 喜剧节拍、克制微表情、避免重复收尾 |
...
特别镜头示例：只有在需求明确写出“电话两端同时显示”“监控墙”“对比画面”时，才把对应 shot 设置为 `frame_layout=split-screen` 或 `triptych`，并写明 `allow_multi_panel=true`。普通人物对话、动作和连续剧镜头全部保持单画面。
...
已知任务反复查询 404 并最终轮询超时时，可以显式使用下面的话术。技能会先检查状态端点和内容端点，任一端仍可用都不会新建任务：

```text
恢复这个项目的 <shot-id>。原 task ID 已经多次轮询超时；请使用 --replace-lost-task --retry-failed --retry-reason "状态和内容端点均确认 404" 做双端点核验。只有两端都确认 404 才创建一个替代任务，并保留旧 task ID 和完整审计记录。
```
