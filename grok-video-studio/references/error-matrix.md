...
| Sub2Api image request uses the T2V key, or T2V uses the image key | Credential role | Configure `sub2api_image_key` and `sub2api_video_key` separately. Legacy `sub2api_key` intentionally supplies both roles only when role-specific values are absent. |
...
| `400` size conflict | Provider parameter repair | NewApi may omit a conflicting `size` while retaining `resolution` and `aspect_ratio`; inspect the recorded provider attempt. |
...
| Sub2Api task reaches a confirmed provider failure | Provider task | Record the failed Sub2Api attempt, then automatically continue with NewApi only when its video key is configured and the failure is not content/account/input related. |
| Sub2Api create is rejected as unsupported or rate-limited before a task exists | Provider routing | It is safe to record a separate NewApi attempt. Keep the same internal request ID and a new attempt ID. |
...
| I2V reference field rejected | Provider contract | Verify Sub2Api JSON uses `image.url` for one first-frame image or `reference_images[].url` for multiple guidance images; NewApi uses repeated multipart `input_reference`. |
